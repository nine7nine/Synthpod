...
_show_cb(LV2UI_Handle instance)
{
	...
	{
		char *executable = "synthpod_sandbox_efl";
		char *const argv [] = {
			executable,
			"-p", handle->plugin_uri,
		...
		execvp(executable, argv); // p = search PATH for executable
	...
	const LV2_Feature *const *features)
{
	...
	if(!handle->plugin_uri || !handle->bundle_path || !handle->ui_uri || !handle->window_title)
	{
