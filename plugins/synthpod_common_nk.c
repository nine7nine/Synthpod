...
_mod_init(plughandle_t *handle, mod_t *mod, const LilvPlugin *plug)
{
	...
	const unsigned num_ports = lilv_plugin_get_num_ports(plug) + 4; // + automation ports

	mod->minimum = sizeof(LV2_Atom_Object);

	for(unsigned p=0; p<num_ports - 4; p++) // - automation ports
	{
	...
	// debug input port
	{
		...
		if(port)
		{
			...
			port->index = p;
			port->port = NULL;
			port->symbol = "__debug__dsp__";
			port->groups = NULL;
			port->name = strdup("DSP Debug Out");
			port->debug = true;
